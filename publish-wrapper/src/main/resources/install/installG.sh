...
#############################解析操作对象#######################
index=$1
. ${GROUP_INSTALLER_HOME}/groupParser.sh install "$index"
...
Usage()
{
  echo 'Usage: installG.sh < sequence_index (deafault:0) >'
}
...
for module in "${MODULES_ARR[@]}"; do
	...
	bash --login --noprofile --norc $MODULE_DIR/install/installM.sh # 考虑到 installM 中的插件可能会 source ~/.bash_profile ~/.profile文件，这里的 --login参数是必须的！【~/.bash_profile ~/.profile只应该在login态使用】
	checkReturn "installG.sh: Fail when install module [$module] ..." "$?"
done

echo ""
echo "###############################"
echo "####  Succ to install ... ####"
echo "###############################"
echo "  Group  : $GROUP_NAME"
echo "  Modules: $MODULES_STR"
echo "###############################"
